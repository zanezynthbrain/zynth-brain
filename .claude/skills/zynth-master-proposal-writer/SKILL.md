---
name: zynth-master-proposal-writer
description: >
  ZYNTH's Master Proposal Writer — turns a brief (or a campaign/event plan) into a
  COMPLETE, precise, professional, WINNING client proposal for any sector. Use this
  whenever the user needs a client-ready proposal, pitch document, or bid — not a
  draft: a systematic, accurate, creative, detailed document engineered to get a YES.
  It covers the full anatomy of a winning proposal (the client's problem in their own
  words, the insight, the big idea, exactly how it's executed, the costed plan, the
  suppliers, the itemised budget, ZYNTH's profit and the sell-vs-sponsor decision, the
  measurable results/ROI, the value shown unmistakably, proof/credibility, risk, terms,
  and a clear call to action). Trigger for "write a proposal", "proposal for [company]",
  "pitch document", "bid", "make this proposal winning", "client-ready proposal",
  "sponsorship proposal", or turning any campaign/event plan into a document a client
  can sign. Grounded per sector (references in zynth-master-campaign-planner) and priced
  at real market rates (MMK/SGD). For the underlying strategy use the campaign/event
  planner skills; this skill packages and sharpens it into a proposal that WINS.
---
...
1. **Capture the brief** — client, sector, objective, budget band, market (MM/SG),
   timing, decision-maker, what "winning the pitch" requires. Missing two+? Ask first.
2. **Get the strategy** — if there isn't already a plan, run
   `zynth-master-campaign-planner` (campaigns) or `zynth-master-event-planner` (events)
   to produce the costed strategy this proposal will package.
...
where it helps (English + Myanmar Unicode/Pyidaungsu).
