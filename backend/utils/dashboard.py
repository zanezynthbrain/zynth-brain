        ...
        creative_jobs = [
            {
                ...
                "title": item.get("title", "Creative job"),
                "status": item.get("status", "pending"),
                "approval_state": (
                    "founder-approved project required"
                    if not (item.get("approval") or {}).get("approved")
                    else "approved route"
                ),
            }
            for item in _cq.pending()[:8]
        ]
    except Exception:
        queue_counts, creative_jobs = {}, []
...
def render_spa() -> str:
    """Render the clean, tablet-first founder command centre.

    The legacy HUD remains below as a historical fallback/reference, while the
    live surface is generated by the focused founder UI module using the same
    authenticated state and API routes.
    """
    from utils.founder_ui import render
    return render(build_state())
