...
def add_batch(rows: Iterable[dict], source: str = "researcher") -> dict:
    ...
    for r in rows:
        ...
        existing.append({
            ...
            # Deep BD intelligence (analytical — inference is fine).
            "company_analysis": (r.get("company_analysis") or "").strip(),
            "online_activities": (r.get("online_activities") or "").strip(),
            "onground_activities": (r.get("onground_activities") or "").strip(),
            "marketing_gap": (r.get("marketing_gap") or "").strip(),
            "zynth_approach": (r.get("zynth_approach") or "").strip(),
            "target_role": (r.get("target_role") or "").strip(),
            "website": (r.get("website") or "").strip(),
            "facebook": (r.get("facebook") or "").strip(),
            # Contact fields — only stored if genuinely provided; never fabricated.
            "contact_name": (r.get("contact_name") or "").strip(),
            "contact_title": (r.get("contact_title") or "").strip(),
